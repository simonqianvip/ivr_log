...
public class ValiImg extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for(int i = 0 ;i<4;i++){
			int num = getRand(-45,45);
			g.rotate(num/180.0*Math.PI, 5+i*30, 22);
			String s = base.charAt(getRand(0,base.length()))+"";
			valiStr.append(s);
			g.drawString(s, 5+i*30, 22);
			g.rotate(-1*num/180.0*Math.PI, 5+i*30, 22);
		}
		
		//释放资源，打出给浏览器
		request.getSession().setAttribute("valiStr", valiStr.toString());
		g.dispose();
